...
void Clear3rdChip_MAX7219(void);											//清第3片MAX7219的显示程序
void Clear4thChip_MAX7219(void);											//清第4片MAX7219的显示程序														
void Clear5thChip_MAX7219(void);											//清第5片MAX7219的显示程序															
void Clear6thChip_MAX7219(void);											//清第6片MAX7219的显示程序															
...
void Write4thChip_MAX7219(uint16_t addr,uint16_t data);				//MAX7219往第4个芯片内写数据程序														
void Write5thChip_MAX7219(uint16_t addr,uint16_t data);				//MAX7219往第5个芯片内写数据程序																
void Write6thChip_MAX7219(uint16_t addr,uint16_t data);				//MAX7219往第6个芯片内写数据程序																
														
														
void DisplayChar_MAX7219 (uint8_t addr,uint8_t data,uint8_t chipno);		//MAX7219显示数据程序
void DisplayDigitalTube1_3Num_MAX7219 (uint16_t data,uint8_t no,uint8_t point); 
...
void Display1_64LED_MAX7219 (uint64_t data); 
void Display2_64LED_MAX7219 (uint64_t data); 
void Display3_64LED_MAX7219 (uint64_t data); 
void Display4_64LED_MAX7219 (uint64_t data); 

void Display_32RowLED_MAX7219 (uint8_t data) ;
...
void MAX7219_Configuration(void)
{
	...
	Clear4thChip_MAX7219();														//清第4片MAX7219的显示程序	
	Clear5thChip_MAX7219();														//清第5片MAX7219的显示程序
	Clear6thChip_MAX7219();														//清第6片MAX7219的显示程序	
...
void Init_MAX7219(void)
{
	...
	Write4thChip_MAX7219(REG_DISPLAY_TEST,0x00);      //选择工作模式（0xX0）			0x01 --test模式
	Write5thChip_MAX7219(REG_DISPLAY_TEST,0x00);      //选择工作模式（0xX0）			0x01 --test模式
	Write6thChip_MAX7219(REG_DISPLAY_TEST,0x00);      //选择工作模式（0xX0）			0x01 --test模式		
	
	Write1stChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度  
	Write2ndChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度 
	Write3rdChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度 	
	Write4thChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度  
	Write5thChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度 
	Write6thChip_MAX7219(REG_INTENSITY,0x0a);        	//设置初始亮度 	
	...
	Write4thChip_MAX7219(REG_SCAN_LIMIT,0x07);        //设置为全显示
	Write5thChip_MAX7219(REG_SCAN_LIMIT,0x07);        //设置为全显示
	Write6thChip_MAX7219(REG_SCAN_LIMIT,0x07);        //设置为全显示		
	...
	Write4thChip_MAX7219(REG_DECODE,0x00);       			//选用B解码模式--0xFF,非解码模式--0x00
	Write5thChip_MAX7219(REG_DECODE,0x00);       			//选用B解码模式--0xFF,非解码模式--0x00
	Write6thChip_MAX7219(REG_DECODE,0x00);       			//选用B解码模式--0xFF,非解码模式--0x00	
	...
	Write4thChip_MAX7219(REG_SHUTDOWN,0x01);         	//开启正常工作模式（0xX1）	0x00 --关闭显示
	Write5thChip_MAX7219(REG_SHUTDOWN,0x01);         	//开启正常工作模式（0xX1）	0x00 --关闭显示	
	Write6thChip_MAX7219(REG_SHUTDOWN,0x01);         	//开启正常工作模式（0xX1）	0x00 --关闭显示			
}
...
/***********************************************************************
函数名称：void Clear4thChip_MAX7219(void);
功    能：清第2片MAX7219的显示程序
输入参数：
输出参数：
编写时间：2014.4.14
...
注    意：
***********************************************************************/
void Clear4thChip_MAX7219(void)
{
  uint8_t i; 
  for (i=1; i <=8; i++) 
	{
		Write4thChip_MAX7219(i, 0x00);
	}
}
...
注    意：
***********************************************************************/
void Clear5thChip_MAX7219(void)
{
  uint8_t i; 
  for (i=1; i <=8; i++) 
	{
		Write5thChip_MAX7219(i, 0x00);
	}
}

/***********************************************************************
函数名称：void Clear6thChip_MAX7219(void);
功    能：清第2片MAX7219的显示程序
输入参数：
输出参数：
编写时间：2014.4.14
...
注    意：
***********************************************************************/
void Clear6thChip_MAX7219(void)
{
  uint8_t i; 
  for (i=1; i <=8; i++) 
	{
		Write6thChip_MAX7219(i, 0x00);
	}
}
...
{ 
	...
	delay(DelayTime_MAX7219);				//对第6个芯片进行空操作	
	Send1Byte_MAX7219(REG_NO_OP); 	//对第6个芯片进行写操作         
	Send1Byte_MAX7219(0x00);
// 	CLK_MAX7219(1);								//时钟上升沿，发送地址		
	delay(DelayTime_MAX7219);				//对第5个芯片进行空操作	
	Send1Byte_MAX7219(REG_NO_OP); 	//对第5个芯片进行写操作         
	Send1Byte_MAX7219(0x00);
// 	CLK_MAX7219(1);								//时钟上升沿，发送地址	
	delay(DelayTime_MAX7219);				//对第4个芯片进行空操作	
	Send1Byte_MAX7219(REG_NO_OP); 	//对第4个芯片进行写操作         
	Send1Byte_MAX7219(0x00);
// 	CLK_MAX7219(1);								//时钟上升沿，发送地址		
...
/***********************************************************************
函数名称：void Write4thChip_MAX7219(uint16_t addr,uint16_t data);
功    能：MAX7219往第4个芯片内写数据程序
输入参数：
输出参数：
编写时间：2014.4.14
编 写 人：
注    意：
***********************************************************************/
void Write4thChip_MAX7219(uint16_t addr,uint16_t data)
{
	LOAD_MAX7219(0); 								//LOAD置低选通MAX7219芯片
...
// 	CLK_MAX7219(1);								//时钟上升沿，发送地址		
	...
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(addr);     		//对第4个芯片进行写操作         
	Send1Byte_MAX7219(data);
//	CLK_MAX7219(1);								//时钟上升沿，发送地址		
	
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(REG_NO_OP);   //对第3个芯片进行写操作         
	Send1Byte_MAX7219(0x00);
//	CLK_MAX7219(1);								//时钟上升沿，发送地址	
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(REG_NO_OP); 	//对第2个芯片进行空操作         
	Send1Byte_MAX7219(0x00);
//	CLK_MAX7219(1);								//时钟上升沿，发送地址
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(REG_NO_OP);		//对第1个芯片进行空操作
	Send1Byte_MAX7219(0x00);
	LOAD_MAX7219(1);  							//LOAD置高禁用MAX7219芯片	
	delay(DelayTime_MAX7219);
}
...
/***********************************************************************
函数名称：void Write5rdChip_MAX7219(uint16_t addr,uint16_t data);
功    能：MAX7219往第5个芯片内写数据程序
输入参数：
输出参数：
编写时间：2014.4.14
编 写 人：
注    意：
***********************************************************************/
void Write5thChip_MAX7219(uint16_t addr,uint16_t data)
{
	LOAD_MAX7219(0); 								//LOAD置低选通MAX7219芯片
...
// 	CLK_MAX7219(1);								//时钟上升沿，发送地址		
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(addr);     		//对第5个芯片进行写操作         
	Send1Byte_MAX7219(data);
//	CLK_MAX7219(1);								//时钟上升沿，发送地址		
...
/***********************************************************************
函数名称：void Write6thChip_MAX7219(uint16_t addr,uint16_t data);
功    能：MAX7219往第6个芯片内写数据程序
输入参数：
输出参数：
编写时间：2014.4.14
编 写 人：
注    意：
***********************************************************************/
void Write6thChip_MAX7219(uint16_t addr,uint16_t data)
{
	LOAD_MAX7219(0); 								//LOAD置低选通MAX7219芯片
	
	delay(DelayTime_MAX7219);
	Send1Byte_MAX7219(addr);     		//对第6个芯片进行写操作         
	Send1Byte_MAX7219(data);
//	CLK_MAX7219(1);								//时钟上升沿，发送地址		
...
void DisplayChar_MAX7219 (uint8_t addr,uint8_t data,uint8_t chipno) 
{
	...
	{
		Write4thChip_MAX7219(addr,data);
	}		
	else if(chipno==5)
	{
		Write5thChip_MAX7219(addr,data);
	}		
	else if(chipno==6)
	{
		Write6thChip_MAX7219(addr,data);
	}			
	
} 

/***********************************************************************
函数名称：void Display1_64LED_MAX7219 (uint64_t data) 
功    能：64个LED显示程序，显示低4位
...
/***********************************************************************
函数名称：void Display2_64LED_MAX7219 (uint64_t data) 
功    能：64个LED显示程序，显示低4位
输入参数：数据值，数码管编号，小数点位置
输出参数：
编写时间：2014.4.14
...
/***********************************************************************
函数名称：void Display3_64LED_MAX7219 (uint64_t data) 
功    能：64个LED显示程序，显示低4位
输入参数：数据值，数码管编号，小数点位置
输出参数：
编写时间：2014.4.14
...
/***********************************************************************
函数名称：void Display4_64LED_MAX7219 (uint64_t data) 
功    能：64个LED显示程序，显示低4位
输入参数：数据值，数码管编号，小数点位置
输出参数：
编写时间：2014.4.14
...
void Display4_64LED_MAX7219 (uint64_t data) 
{
	...
	Init_MAX7219();					//需要重复设置，否则出现不显示现象
	code=data%0x100;												
	Write4thChip_MAX7219(DIG_1,code);	
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_2,code);		
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_3,code);	
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_4,code);
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_5,code);	
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_6,code);	
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_7,code);	
	data=data/0x100;
	code=data%0x100;		
	Write4thChip_MAX7219(DIG_8,code);	
}

/***********************************************************************
函数名称：void Display_32RowLED_MAX7219 (uint64_t data) 
功    能：64个LED显示程序，显示低4位
输入参数：数据值，数码管编号，小数点位置
输出参数：
编写时间：2014.4.14
...
void Display_32RowLED_MAX7219 (uint8_t data) 
{
	...
	switch(data%8)
	{
		...
		case 0 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x0000000000000000);
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==1)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x0000000000000000);					
			}			
			else if(data/8==4)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);					
			}											
			break;
		}		
		case 1 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x0101010101010101); 
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x0101010101010101); 
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x0101010101010101); 
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x0101010101010101); 					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 2 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x0303030303030303);
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x0303030303030303);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x0303030303030303);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x0303030303030303);					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 3 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x0707070707070707); 
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x0707070707070707); 
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x0707070707070707); 
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x0707070707070707); 					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 4 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x0F0F0F0F0F0F0F0F); 
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x0F0F0F0F0F0F0F0F); 
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x0F0F0F0F0F0F0F0F); 
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x0F0F0F0F0F0F0F0F); 					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 5 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x1F1F1F1F1F1F1F1F); 
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x1F1F1F1F1F1F1F1F); 
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x1F1F1F1F1F1F1F1F); 
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x1F1F1F1F1F1F1F1F); 					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 6 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x3F3F3F3F3F3F3F3F);
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x3F3F3F3F3F3F3F3F);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x3F3F3F3F3F3F3F3F);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x3F3F3F3F3F3F3F3F);					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
		...
		case 7 :
		{
			if(data/8==0)
			{
				Display1_64LED_MAX7219 (0x7F7F7F7F7F7F7F7F);
				Display2_64LED_MAX7219 (0x0000000000000000);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			...
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0x7F7F7F7F7F7F7F7F);
				Display3_64LED_MAX7219 (0x0000000000000000);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}
			else if(data/8==2)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0x7F7F7F7F7F7F7F7F);
				Display4_64LED_MAX7219 (0x0000000000000000);				
			}			
			else if(data/8==3)
			{
				...
				Display2_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display3_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
				Display4_64LED_MAX7219 (0x7F7F7F7F7F7F7F7F);					
			}			
			else if(data/8==4)
			{
				Display1_64LED_MAX7219 (0xFFFFFFFFFFFFFFFF);
...
/***********************************************************************
函数名称：void DisplayDigitalTube1_3Num_MAX7219 (uint16_t data,uint8_t no,uint8_t point) 
功    能：第1、2号4位数码管显示程序，显示低3位
输入参数：数据值，数码管编号，小数点位置
输出参数：
编写时间：2014.4.14
编 写 人：
