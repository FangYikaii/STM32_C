...
void Uart_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RS485_COM4_RCC_APB2Periph_EnGPIOX, ENABLE); 					 
			
	GPIO_InitStructure.GPIO_Pin = RS485_COM4_EnPIN;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(RS485_COM4_EnGPIOX, &GPIO_InitStructure);
	RS485_COM4_RxEnable(1);																			//RS485_COM2处于接收状态
		...
		if(Rec_COM2.Data[0]==0x3a)
		{
			if(Rec_COM2.Count>=4)
			{
				if(Rec_COM2.Data[Rec_COM2.Count-1]==0x0A && Rec_COM2.Data[Rec_COM2.Count-2]==0x0D)
				{
					...
					Rec_COM2.TimeOut=0;											//串口溢出计时
					flagCOM1.Bits.Rec_Flag=FALSE;					//串口溢出计时				
