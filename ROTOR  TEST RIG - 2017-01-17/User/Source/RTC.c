...
/*
RTC(Real-Time Clock)实时时钟：特点是掉电后继续运行，利用VDD（电源适配器或usb线）和VBAT（锂电池）给RTC供电。
RTC:32位计数器，向上计数，晶振32.678KHZ
*/
...
struct DATE NowTime;                                                  //定义结构体-日期结果
volatile union FlagRTCA flagRTC;                                      //定义联合体-日期结果-标志位
//========================================
//函数声明
...
{
  /* Enable PWR and BKP clocks 使能外部时钟*/
...
// 			NVIC_PriorityGroupConfig(NVIC_PriorityGroup_3);			//抢断优先级 0-7    响应优先级 0-1	
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 14;          //抢断优先级14
...
uint8_t RTC_SetTime(uint16_t year,uint8_t mon,uint8_t day,uint8_t hour,uint8_t min,uint8_t sec)
{
	...
	for(t=0;t<mon;t++)					//把前面月份的秒钟相加
	{
		...
		if(Is_Leap_Year(year)&&t==1)					//闰年2月份多1天的秒钟数
		{
			seccount+=86400;			
		}
	}
