...
void USBH_MSC_HandleBOTXfer(void)
{
  ...
  if(HCD_IsDeviceConnected(&USB_OTG_FS_dev))
  {  
    URB_Status = HCD_GetURB_State(&USB_OTG_FS_dev); 
    
    switch (USBH_MSC_BOTXferParam.BOTState)
    {
    case USBH_MSC_SEND_CBW:
      /* send CBW */    
      USBH_BulkSendData (&USB_OTG_FS_dev,
    ...
    case USBH_MSC_BOT_DATAIN_STATE:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        if(remainingDataLength > USBH_MSC_PAGE_SIZE_64)
        {
          USBH_BulkReceiveData (&USB_OTG_FS_dev,
	                        datapointer, 
			        USBH_MSC_PAGE_SIZE_64 , 
			        MSC_Machine.hc_num_in);
          
          remainingDataLength -= USBH_MSC_PAGE_SIZE_64;
          datapointer = datapointer + USBH_MSC_PAGE_SIZE_64;
        }
    ...
    case USBH_MSC_BOT_DATAOUT_STATE:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        if(remainingDataLength > USBH_MSC_PAGE_SIZE_64)
        {
          USBH_BulkSendData (&USB_OTG_FS_dev,
                             ...
                             MSC_Machine.hc_num_out);
          datapointer = datapointer + USBH_MSC_PAGE_SIZE_64;
          remainingDataLength = remainingDataLength - USBH_MSC_PAGE_SIZE_64;
        }
        else if ( remainingDataLength == 0)
        {
          /* If value was 0, and successful transfer, then change the state */
      ...
      {
        USBH_BulkSendData (&USB_OTG_FS_dev,
	                   (datapointer - USBH_MSC_PAGE_SIZE_64), 
			   USBH_MSC_PAGE_SIZE_64 , 
			   MSC_Machine.hc_num_out);
      }
    ...
    case USBH_MSC_DECODE_CSW:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        USBH_MSC_BOTXferParam.BOTXferStatus = USBH_MSC_DecodeCSW();
      }
      else if(URB_Status == URB_STALL)     
      {
        error_direction = USBH_MSC_DIR_IN;
    ...
    case USBH_MSC_BOT_ERROR_IN: 
      status = USBH_MSC_BOT_Abort(USBH_MSC_DIR_IN);
      if (status == USBH_OK)
      {
    ...
    case USBH_MSC_BOT_ERROR_OUT: 
      status = USBH_MSC_BOT_Abort(USBH_MSC_DIR_OUT);
      if ( status == USBH_OK)
...
USBH_Status USBH_MSC_BOT_Abort(uint8_t direction)
{
  ...
  case USBH_MSC_DIR_IN :
    ...
    status = USBH_ClrFeature((&USB_OTG_FS_dev), 
...
uint8_t USBH_MSC_DecodeCSW(void)
{
  ...
  if(HCD_IsDeviceConnected(&USB_OTG_FS_dev))
  {
    ...
    dataXferCount = HCD_GetXferCnt(&USB_OTG_FS_dev); 
